...
with open("../dataset/semeval.csv", 'r') as main_dataset:
    ...
    pos_train_completed = neg_train_completed = neu_train_completed =\
        pos_tune_completed = neg_tune_completed = neu_tune_completed = False
    count = 0
    ...
    for line in main:
        ...
        if count % 3 == 2:
            if line[1] == "positive" and pos_count < globals.POS_RATIO * TOTAL_TRAIN_DATA:
                POS_TUNE_DATA.update({str(count): line[2]})
                pos_count += 1
            elif line[1] == "positive" and pos_count > globals.POS_RATIO * TOTAL_TRAIN_DATA:
                pos_tune_completed = True
            if line[1] == "negative" and neg_count < globals.NEG_RATIO * TOTAL_TRAIN_DATA:
                NEG_TUNE_DATA.update({str(count): line[2]})
                neg_count += 1
            elif line[1] == "negative" and neg_count > globals.NEG_RATIO * TOTAL_TRAIN_DATA:
                neg_tune_completed = True
            if line[1] == "neutral" and neu_count < globals.NEU_RATIO * TOTAL_TRAIN_DATA:
                NEU_TUNE_DATA.update({str(count): line[2]})
                neu_count += 1
            elif line[1] == "neutral" and neu_count > globals.NEU_RATIO * TOTAL_TRAIN_DATA:
                neu_tune_completed = True

        else :
            if line[1] == "positive" and pos_count < globals.POS_RATIO * TOTAL_TRAIN_DATA:
                POS_TRAIN_DATA.update({str(count): line[2]})
                pos_count += 1
            elif line[1] == "positive" and pos_count > globals.POS_RATIO * TOTAL_TRAIN_DATA:
                pos_train_completed = True
            if line[1] == "negative" and neg_count < globals.NEG_RATIO * TOTAL_TRAIN_DATA:
                NEG_TRAIN_DATA.update({str(count): line[2]})
                neg_count += 1
            elif line[1] == "negative" and neg_count > globals.NEG_RATIO * TOTAL_TRAIN_DATA:
                neg_train_completed = True
            if line[1] == "neutral" and neu_count < globals.NEU_RATIO * TOTAL_TRAIN_DATA:
                NEU_TRAIN_DATA.update({str(count): line[2]})
                neu_count += 1
            elif line[1] == "neutral" and neu_count > globals.NEU_RATIO * TOTAL_TRAIN_DATA:
                neu_train_completed = True

        if pos_train_completed and pos_tune_completed and \
            neg_train_completed and neg_tune_completed and \
            neu_train_completed and neu_tune_completed : \
            break


POS_UNI_GRAM,POS_POST_UNI_GRAM = ngram.ngram(POS_TRAIN_DATA,1)
NEG_UNI_GRAM,NEG_POST_UNI_GRAM = ngram.ngram(NEG_TRAIN_DATA,1)
NEU_UNI_GRAM,NEU_POST_UNI_GRAM = ngram.ngram(NEU_TRAIN_DATA,1)
pos_vec, pos_lab = load_matrix_sub(POS_TRAIN_DATA,2.0)
neg_vec, neg_lab = load_matrix_sub(NEG_TRAIN_DATA,-2.0)
neu_vec, neu_lab = load_matrix_sub(NEU_TRAIN_DATA,0.0)
...
vectors_train = vectors
labels_train = labels

POS_UNI_GRAM,POS_POST_UNI_GRAM = ngram.ngram(POS_TUNE_DATA,1)
NEG_UNI_GRAM,NEG_POST_UNI_GRAM = ngram.ngram(NEG_TUNE_DATA,1)
NEU_UNI_GRAM,NEU_POST_UNI_GRAM = ngram.ngram(NEU_TUNE_DATA,1)
pos_vec, pos_lab = load_matrix_sub(POS_TUNE_DATA,2.0)
neg_vec, neg_lab = load_matrix_sub(NEG_TUNE_DATA,-2.0)
neu_vec, neu_lab = load_matrix_sub(NEU_TUNE_DATA,0.0)
vectors = pos_vec + neg_vec + neu_vec
labels = pos_lab + neg_lab + neu_lab
vectors_scaled = pr.scale(np.array(vectors))
scaler = pr.StandardScaler().fit(vectors)
...
def get_best_svm():
    ...
    svr.fit(vectors_train, labels_train)
    grid = GridSearchCV(svr, parameters, scoring='f1_micro', cv=5, n_jobs=-1)
    tuned_model = grid.fit(vectors_tune, labels_tune)
